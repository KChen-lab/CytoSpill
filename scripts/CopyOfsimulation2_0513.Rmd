...
```{r}
library(CATALYST)
packageVersion("CATALYST")

#code adapted from CATALYST example Helena L Crowell et al. https://bioconductor.org/packages/release/bioc/vignettes/CATALYST/inst/doc/preprocessing.html#computespillmat-estimation-of-the-spillover-matrix
...
comped_nnls_exprs <- flowCore::exprs(comped_nnls)

#remove unaffected channel
comped_nnls_exprs <- comped_nnls_exprs[,c(2, 4:19, 21:39)]
...
estimate_sm <- function(data, spill_cols){
  ...
  for (i in seq_along(data)){
    cutoffs_sim <- .DeriveCutoffs(data=data[[i]], cols=c(1:ncol(data[[i]])), n = 5000, flexrep = 3)
    threshold <- 0.1
...
# }
set.seed(1)
estimate_sm_simulated <- estimate_sm(data = data_simulated, spill_cols = spill_cols_data(comped_nnls_exprs))
...
for (i in seq_along(nmf_estimate_sm)){
  ...
  nmf_estimate_sm[[i]] <- nmf_estimate_sm[[i]] + diag(36)
}
```
...
slsqp_comparison_noboundary <- as.data.frame(slsqp_comparison[which(slsqp_comparison[,2]!=0 & slsqp_comparison[,2]!=0.1),])
slsqp_comparison_noboundary <- as.data.frame(slsqp_comparison)
slsqp_comparison_noboundary<- slsqp_comparison_noboundary[which(slsqp_comparison_noboundary$my_method > 0),]
slsqp_comparison_noboundary<- slsqp_comparison_noboundary[which(slsqp_comparison_noboundary$my_method < 0.0999),]
...
ggplot(as.data.frame(slsqp_comparison), aes(x=my_method, y=simulation)) + geom_point(size=0.1) + theme_classic() + xlab('Sequential quadratic programming estimated spillover effects') + ylab('Simulated spillover effects') + coord_fixed() +geom_smooth()
...
ggplot(as.data.frame(nmf_comparison), aes(x=my_method, y=simulation)) + geom_point(size=0.1) + theme_classic() + xlab('NMF estimated spillover effects') + ylab('Simulated spillover effects') + coord_fixed() + geom_smooth()
```
```{r}
