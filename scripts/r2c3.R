...
r2c3_simulation_data <- function(){
  Nd142Di <- sample(c(rnorm(10000, 200, 50), rep(0, 10000)), 20000)
  Nd142Di[which(Nd142Di < 0)] <- 0

  Nd144Di <- sample(c(rnorm(10000, 300, 75), rep(0, 10000)), 20000)
  Nd144Di[which(Nd144Di < 0)] <- 0

  Nd145Di <- sample(c(rnorm(10000, 400, 100), rep(0, 10000)), 20000)
  Nd145Di[which(Nd145Di < 0)] <- 0

  Nd143Di <- sample(c(rnorm(10000, 300, 75), rep(0, 10000)), 20000)
...
ggplot(as.data.frame(withvalue_comparison), aes(x=my_method, y=simulation)) +
  ...
  ylab('Simulated spillover effects') + coord_fixed()
...
cor(withvaluevswithoutvalue[,1],withvaluevswithoutvalue[,2])^2
#0.2345154
